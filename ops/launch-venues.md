...
2026-08-10, 2026-08-14, 2026-08-15, 2026-08-20, and 2026-08-21 (`/pricing/`
...
- **G2 — NEEDS_NISH_STEP (agent-executed submission declined).** The
  ...
  surfaced for the free profile; paid Marketing Solutions stay a separate
   spend call. (Lane attempts 2026-08-15, 2026-08-21, and 2026-08-23: NOT
   EXECUTED — see the G2 section below.)
...
- Next action (unchanged): Nish submits manually using the kits below (Product
  Hunt: choose/confirm the launch date and publish; BetaList: decide whether to
  pay and submit), then this file should be updated with the public URL
  and the venue's status line flipped to live.

### Fleet lane attempt 2026-08-21 (Product Hunt + BetaList — NOT EXECUTED, decisions re-verified; guard verified live for the first time)

- Attempted by lane 1 (packet item 7133745c8e, re-dispatch: "List the product
  on Product Hunt and record a submit-or-decline decision for BetaList —
  baseline launch venue never live"). Neither listing was submitted and
  neither is live; the 2026-08-09 / 2026-08-14 / 2026-08-15 / 2026-08-20
  decisions stand unchanged and both kits remain valid and copy-paste ready:
  - **Product Hunt — submit declined by the fleet (NEEDS_NISH_STEP).**
    ...
    updated 2026-08-08, well within the 90-day freshness window) still marks
    `producthunt.com` `automation_disposition: prohibited` and the
    executable `allowlist` is still empty. **The `venue-claim` binary is now
    installed** (`/home/nish/.local/bin/venue-claim`), so this run verified
    the gate live instead of inferring it: `venue-claim claim producthunt.com
    aiconverter-app …` exited **4** ("ALLOWLIST/POLICY BLOCK: venue
    producthunt.com is reviewed as prohibited - not automation-allowed;
    route to NEEDS-NISH/manual, never bypass") and created **no ledger
    record** (`venue-claim list` shows zero aiconverter records before and
    after). Per the contract, a blocked exit means NO browser work; the
    launch flow remains account-gated and publishing stays a human action
    with Nish.
  - **BetaList — DECLINE (SKIPPED_PAID, re-verified 2026-08-21).** The gate
    was likewise verified live: `venue-claim claim betalist.com
    aiconverter-app …` exited **4** ("unknown (not allowlisted, not
    reviewed). No current official policy evidence permits automated
    action"), no ledger record created. The official Support page (re-fetched
    live 2026-08-21) still states "All submissions are paid. There is no
    free submission option." with the FAQ verbatim "No. BetaList used to
    offer free submissions, but all submissions now require payment." The
    fleet declines the paid submission (spend decisions stay with Nish); no
    spend authorization exists in `agent-state/authorizations/` (still only
    the sol-xhigh worker grant — expired 2026-08-14).
- Live re-verification (2026-08-21):
  - Product Hunt: the search page remains Cloudflare-challenged for plain
    HTTP from this VPS — `https://www.producthunt.com/search?q=aiconverter`
    returns **403** (same as the 2026-08-14/15/20 runs; no JS renderer was
    available in this lane run, so the Camoufox-rendered finding of
    2026-08-14/15/20 — no aiconverter.app result — is carried forward).
    Credential-free secondary checks (2026-08-21) found **no evidence of
    any listing**: Wayback availability API returns no archived snapshots
    for producthunt.com/products/aiconverter, /products/ai-converter, or
    /products/aiconverter-app, and a Wayback CDX domain query
    (`url=producthunt.com&matchType=domain&filter=urlkey:.*aiconverter.*`)
    returns **zero** archived Product Hunt URLs containing "aiconverter".
    The venue hosts the category (receipt-ai, ledgerbox per the 2026-08-09
    evidence); the listing is missing.
  - BetaList search `q=aiconverter`
    (https://www.betalist.com/search?q=aiconverter, HTTP 200): still "No
    results found for aiconverter" — no duplicate, no listing.
  - BetaList support page (https://www.betalist.com/support, HTTP 200):
    paid-only statements verbatim unchanged (see above); eligibility
    unchanged: "Your startup must have its own domain—we don't accept
    submissions using free hosting subdomains (like vercel.app,
    netlify.app, herokuapp.com) or direct links to app stores."
  - BetaList submit page (https://betalist.com/submit, HTTP 200): unchanged
    from 2026-08-20 — form is account-gated; the support page remains the
    authoritative paid-only statement.
  - Kit reference pages all live HTTP 200 (2026-08-21): `/`, `/llms.txt`,
    `/bank-statement-pdf-to-csv/`, `/sample-csv/`, `/trust/`, `/formats/`.
    `/pricing/` is **still 404** (regression first noted 2026-08-20,
    unchanged) and `/receipt-to-csv/` still 404 (the kit never claimed it).
- Next action (unchanged): Nish submits manually using the kits below (Product
...
### Decision (dated 2026-08-10, re-verified 2026-08-11, 2026-08-12, 2026-08-14, 2026-08-20 and 2026-08-22)
...
### Fleet lane attempt 2026-08-22 (WeLikeTools — NOT EXECUTED, re-verified)

- Attempted by lane 1 (packet item c716f1de42, re-run: "List the product on
  the free WeLikeTools directory and record a paid/decline decision for
  xix.ai"). The listing was **still not submitted**: the decision above still
  binds and no policy or authorization change occurred since 2026-08-20.
  `agent-state/growth-loop/venue-policy.json` (updated 2026-08-08) still has
  no weliketools.com entry — `automation_disposition: unknown`, allowlist
  still empty — so `venue-claim claim` exits 4 and the agent must not drive a
  browser submission. The Google sign-in gate also stands (re-verified live
  2026-08-22: `https://weliketools.com/submit` still renders "Log in to
  Submit" / "Log in with Google"). Authorizations still hold only the
  sol-xhigh-worker-grant. The `venue-claim` binary is now installed in the
  lane environment (live 2026-08-22: `venue-claim claim weliketools.com
  aiconverter-app` exited 4 — "ALLOWLIST/POLICY BLOCK: venue weliketools.com
  is unknown (not allowlisted, not reviewed)"); the policy JSON remains the
  authoritative guard and is unchanged; this record is the honest
  NOT-EXECUTED lane outcome the packet requires.
- Live re-verification (2026-08-22, plain HTTP):
  - No duplicate: search `q=aiconverter`
    (https://weliketools.com/search?q=aiconverter) — HTTP 200, "There are 0
    tools found for aiconverter" — no aiconverter.app listing anywhere.
  - Exact-category competitor still live:
    https://weliketools.com/tool/bankstatementengine — HTTP 200, "Bank
    Statement Engine", Category: Business & Finance (unchanged).
  - Submit page live, HTTP 200: https://weliketools.com/submit — "Log in to
    Submit" / "Log in with Google" gate; no fee or paid tier mentioned
    anywhere on the page (unchanged; free).
  - Terms (https://weliketools.com/terms, HTTP 200): prohibited conduct
    still includes "Using automated tools to scrape or harvest data from our
    website" (unchanged; flag for the venue research desk, guard stays
    exit-4 either way).
  - Kit reference pages all live HTTP 200 (2026-08-22): `/`, `/llms.txt`,
    `/bank-statement-pdf-to-csv/`, `/sample-csv/`, `/trust/`, `/formats/`.
    Change vs prior runs: `/pricing/` and `/receipt-to-csv/` are now HTTP
    200 (were 404 through 2026-08-20); the kits still claim none of those
    routes.
- Next action (unchanged): Nish signs in with Google and submits using the
  kit above. The only route to an agent-executed submission would be the
  venue research desk reviewing weliketools.com and adding it to the policy
...
### Fleet lane attempt 2026-08-22 (xix.ai — decision re-recorded, NOT EXECUTED)

- Attempted by lane 1 (packet item c716f1de42, re-run: "record a
  paid/decline decision for xix.ai"). The dated decision stands as recorded
  above — **PAID listing at $9.90 recommended for evaluation; declined for
  agent-executed submission** — and no policy or authorization change
  occurred since 2026-08-20: `agent-state/growth-loop/venue-policy.json`
  (updated 2026-08-08) still has no xix.ai entry (`automation_disposition:
  unknown`, allowlist still empty), so `venue-claim claim` exits 4 and the
  agent must not drive the browser submission; the $9.90 spend and the
  account creation remain Nish's human actions, and
  `agent-state/authorizations/` still holds only the sol-xhigh-worker-grant
  (no xix.ai entry). The `venue-claim` binary is now installed in the lane
  environment (live 2026-08-22: `venue-claim claim xix.ai aiconverter-app`
  exited 4 — "ALLOWLIST/POLICY BLOCK: venue xix.ai is unknown (not
  allowlisted, not reviewed)"); the policy JSON remains the authoritative
  guard and is unchanged; this record is the honest NOT-EXECUTED lane
  outcome the packet requires (the dated decision line above flips to
  SUBMITTED or DECLINED only after Nish decides on the spend).
- Live re-verification (2026-08-22, plain HTTP):
  - No duplicate: site search `q=aiconverter`
    (https://xix.ai/search?q=aiconverter) — HTTP 200, "No results found in
    the search", no aiconverter.app hit in the page;
    https://xix.ai/tool/ai-converter.html — HTTP 404 ("Page not found").
  - Exact-term category page still live and still competitor-occupied:
    https://xix.ai/tool/pdf-bank-statements-converter.html — HTTP 200,
    "PDF Bank Statements Converter" (current tool aibankparser.com,
    unchanged from the 2026-08-11/12/14/20 record).
  - Submit page still payment-gated, HTTP 200: https://xix.ai/submit —
    "Total: $ 9.90", "Pay $ 9.90", "Amount: $ 9.90", "No queue, listed
    within 48 hours", sign-in required, graphic captcha at the
    login/payment step — unchanged (live text is `$ 9.90` with a space).
  - Kit reference pages all live HTTP 200 (2026-08-22): `/`, `/llms.txt`,
    `/bank-statement-pdf-to-csv/`, `/sample-csv/`, `/trust/`, `/formats/`.
    Change vs prior runs: `/pricing/` and `/receipt-to-csv/` are now HTTP
    200 (were 404 through 2026-08-20); the kits still claim none of those
...
### Fleet lane attempt 2026-08-20 (Toolbit.ai — NOT EXECUTED, re-verified)

- Attempted by lane 1 (packet item c9b3592b7b re-dispatch: "List the
  product on Toolbit.ai (free plan first; record paid/verified decision) —
  venue already lists exact-category peers"). The listing was **still not
  submitted**: the 2026-08-17 decision above still binds and no policy or
  authorization change occurred since 2026-08-17.
  `agent-state/growth-loop/venue-policy.json` (updated 2026-08-08, mtime
  unchanged at 2026-08-08T06:35:38Z) still has no toolbit.ai entry —
  `automation_disposition: unknown`, allowlist still empty (only the legacy
  producthunt.com | seo-fix-kit claim, with `status: expired`, is in the
  ledger; reviewed_venues holds only producthunt.com (prohibited), g2.com,
  capterra.com, getapp.com, alternativeto.net, startupsubmit.app (all
  `unknown`)) — so per the `venue-claim` contract, `claim` exits 4 and "A
  blocked exit means NO browser work." The free flow is also still a human
  account action (sign-in wall at `/submit/tool?plan=free`, redirecting to
  `/login?redirect=%2Fsubmit%2Ftool`) whose verification step (embedding
  the Launch Badge) is account-gated too, and the ToS still prohibits
  automated access (clause 0.2 in the rendered HTML — "Use any robot,
  spider, or other automatic device, process, or means to access Service
  for any purpose, including monitoring or copying any of the material on
  Service" — same class of language as Product Hunt's prohibition). The
  `venue-claim` binary is not installed in the lane environment
  (`/home/nish/.local/bin/venue-claim: No such file or directory`), but the
  policy JSON is the authoritative guard and is unchanged; this record is
  the honest NOT-EXECUTED lane outcome the packet requires.
- Live re-verification 2026-08-20 (plain HTTPS GETs — toolbit.ai apex
  still 301s to `https://www.toolbit.ai/`, server `cloudflare`; the `www.`
  host serves curl without a Cloudflare challenge):
  - No duplicate: site search `q=aiconverter`
    (https://www.toolbit.ai/search?q=aiconverter, HTTP 200, `<title>aiconverter
    - AI Tools Search</title>`) returns zero `aiconverter.app` mentions in
    the rendered HTML — the only `aiconverter` strings are the query echo,
    the title, and the meta description "Compare features, pricing, and
    reviews of tools matching your search."
  ...
  - `/submit/tool?plan=free` (HTTP 307) still redirects to
    ...
    noted GitHub; today's static render surfaces Google only, the rest of
    the OAuth row is JS-rendered). Either way, the sign-in wall is
  ...
  - Kit reference pages all live HTTP 200 (2026-08-20): `/`,
    ...
    404 (unchanged; the kit claims none of those routes).
- Policy ledger unchanged (re-read 2026-08-20): `venue-policy.json`
  ...
  alternativeto.net, startupsubmit.app (all `unknown`) — toolbit.ai is in
  neither list.
- `agent-state/authorizations/` holds only
  `sol-xhigh-worker-grant-20260811.json` — no toolbit.ai spend
  authorization exists.
- Paid/verified decision (re-recorded 2026-08-20, unchanged): **free
  community listing first** — "Launch Tool $0 / Forever" with Launch
  Badge verification, reviewed in up to 3 days, permanent directory
  listing. The paid **Launch Tool $29 / One-time** (listed within 24h,
  blue verified badge, sidebar featured 1 day, one X post) stays
  deferred to Nish's spend call. The free listing's "verified" step —
  embedding Toolbit's Launch Badge snippet on aiconverter.app — is a
  follow-up owner action that needs a tiny deploy once Nish has the
  snippet from the submission flow (the snippet is account-gated and
  `/launch-badge` still 404).
- Next action (unchanged, human-owned): Nish signs in (Google or
  GitHub) and submits using the kit above, embeds the Launch Badge
  snippet on aiconverter.app (tiny deploy) to complete the free-verified
  listing, then this file should be updated with the public tool URL.
  The only route to an agent-executed submission would be the venue
  research desk reviewing toolbit.ai's ToS (which, like Product Hunt's,
  prohibits automated access) and adding the venue to the policy
  allowlist. After the listing, confirm the tool appears in search
  `q=aiconverter` on toolbit.ai and flip this venue's status line to
  live.
...
### Fleet lane attempt 2026-08-23 (G2 — NOT EXECUTED, decision re-verified)

- Attempted by lane 1 (packet item 4eb99c12cf, same item as the 2026-08-15 and
  2026-08-21 attempts). The eligibility condition is **still MET** and the
  venue policy guard + reviewed vendor form continue to block agent-executed
  submission. `agent-state/growth-loop/venue-policy.json` (re-read 2026-08-23,
  still dated 2026-08-08) lists g2.com with `automation_disposition: "unknown"`
  and the executable `allowlist` is **still empty** — so the `venue-claim`
  contract still produces an exit-4 "Allowlist/policy block" and "A blocked
  exit means NO browser work" applies unchanged. The `venue-claim` binary is
  now installed on this VPS, but the policy JSON is the authoritative guard
  and is unchanged.
- Live re-verification (2026-08-23, credential-free):
  - Production workflow live, all HTTP 200: `/`, `/llms.txt`,
    `/bank-statement-pdf-to-csv/`, `/sample-csv/`, `/trust/`, `/formats/`.
    Bank-statement copy still confirms released/non-beta/B2B framing
    ("Convert bank statement PDFs into reviewable CSV rows with
    preview-first extraction and short private file retention"; "Preview
    before payment"; "No bank login, no email intake, no manual review
    queue"). `/pricing/` and `/receipt-to-csv/` are also live (HTTP 200
    as of 2026-08-23; both `public/pricing/index.html` and
    `public/receipt-to-csv/index.html` exist in the workspace), but the kit
    does not claim them; the kit's canonical links remain `/`,
    `/bank-statement-pdf-to-csv/`, `/sample-csv/`, `/trust/`, `/formats/`.
  - G2 eligibility rule live: https://documentation.g2.com/help/docs/
    finding-or-listing-a-product-on-g2 → HTTP 200 (B2C/alpha-beta exclusion
    quote unchanged).
  - Official flow live: https://sell.g2.com/create-a-profile → HTTP 200
    ...
    https://sell.g2.com/claim-your-profile → "Claim that thing for free"
    (unchanged).
  - The G2 free profile path is **not** `app.g2digitalmarkets.com/get-listed/start`;
  ...
  - Direct g2.com access (re-checked 2026-08-23, plain Mozilla/Chrome UA):
    `https://www.g2.com/` → HTTP **403** (bot-walled; same as 2026-08-15 and
    2026-08-21).
- Decision: **unchanged.** The kit below is still copy-paste ready and the
  next step is still Nish's manual Product Submission Form + claim. No new
  G2 profile for aiconverter.app / "AI Converter" has been published since
  2026-08-21 (the live peer profile
  https://www.g2.com/products/bank-statement-converter-ai-online/reviews
  remains the only exact-category listing on G2).
- Full evidence: `.lane/reports/lane1-g2-listing-20260823.md` (this run);
  the earlier 2026-08-15 and 2026-08-21 reports remain valid and the kit
  below is carried forward with the corrected `/pricing/` / `/receipt-to-csv/`
  rationale.
...
- Post-listing check: the public profile resolves and links
  ...
  `/trust/`, `/formats/`; then update this file with the public URL
  (g2.com/products/{slug}).
