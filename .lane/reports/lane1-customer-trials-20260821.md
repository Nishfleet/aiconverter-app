# Lane 1 — Customer trials (scout item) re-verification run 3, 2026-08-21
...
be. The binding verdict from PR #58 / #112 / #128 (`ops/customer-trials.md`)
still holds: recruiting real bookkeepers / SMB operators requires Nish's human
...
repo code as of 2026-08-21, and the grant decision is re-recorded unchanged.
The trial ledger in `ops/customer-trials.md` still has **0/5 rows** — no
session has been run yet, so the scout item stays open.

## Live / repo re-verification 2026-08-21 (run 3)

- Live `/api/health` (curl, 2026-08-21):
  `{"ok":true,...,"dodo":{...,"freeDownloads":false}}` — free full export is
  **still NOT enabled** in production; the gate is real and would block a
  trial download today absent the per-job grant. Gate code unchanged:
...
- Repo baseline: `npm run check:pricing` → "Pricing is consistent.";
  `node --test tests/*.test.mjs` → 220/220 pass.
...
- Working tree is at fresh `origin/main` (`141ab98`, 2026-08-20 19:35 +0530);
  prior run's PR #128 is merged.

## What exists (delivered PR #58, re-verified #112, #128, and again here)
...
- Grant decision (dated 2026-08-11, re-recorded unchanged 2026-08-21):
...
- `.lane/reports/lane1-customer-trials-20260821.md` — this honest
