# Lane 1 report — Uneed free waiting-line submit (2026-08-17 re-verify)
...
The listing was **not submitted** — the seventh lane run to reach this
...
2026-08-16 re-verify, and this 2026-08-17 re-verify). Both independent
blockers are unchanged and were re-verified live this run:
...
1. **Fleet venue policy gate (machine-enforced, unchanged).**
   ...
   empty (`allowlist: {}`). Reviewed venues list stays at `producthunt.com,
   g2.com, capterra.com, getapp.com, alternativeto.net, startupsubmit.app`.
   Per the `venue-claim` contract, `venue-claim claim uneed.best
...
`sol-xhigh-worker-grant-20260811.json` grant (expired 2026-08-14); there is
no Uneed spend authorization and no Uneed entry in any dispatch ledger. The
free tier costs nothing — the blocker is the account gate, not money — but
STWL dates remain bookable (soonest **2026-08-20**, live this run) and the
launch guide text is unchanged.

## Live re-verification (2026-08-17, plain HTTP — Uneed is curl-friendly, unlike Toolify)

All checks below are fresh live HTTP fetches from this lane run.

| Check | Result |
|---|---|
| `GET /api/v1/launch-dates` (public, follows `uneed.best` → `www.uneed.best` redirect) | HTTP 200. `free_next_available: 2027-02-14` (**181 days out**, ~6 months, unchanged from 2026-08-16). STWL dates bookable from **2026-08-20** (soonest). |
| No duplicate (public search API `q=aiconverter`) | `mcp.uneed.best/v1/search?q=aiconverter` → no aiconverter.app result — unrelated converters only (TailConverter, Heic Converter, File Converter — Iconscout, SVG Converter, ...). |
...
Note: `uneed.best/...` issues a 302 → `www.uneed.best/...` redirect; raw
status without `-L` is 302 (the prior 2026-08-15 run hit the same redirect
but reported 200 because it followed with `-L`). The live search endpoint
lives on the mcp host (`mcp.uneed.best/v1/search`), which is what the ops
doc already references. `www.uneed.best/api/v1/search?q=...` still returns
404.
...
attempt, the 2026-08-15 NOT EXECUTED attempt, the 2026-08-15 re-verify (PR #118),
the 2026-08-16 NOT EXECUTED re-verify (PR #109), and the 2026-08-16 re-verify
landing in `ops/launch-venues.md` § Uneed. Every fact this run re-verified
matches those records exactly (free date 2027-02-14 unchanged, STWL soonest
2026-08-20 unchanged, no duplicate still true, 5 peers still 200). Adding a
seventh dated section to the shared `ops/launch-venues.md` would conflict
with sibling lanes' open PRs on that file (34 fleet PRs are dead on exactly
this) for zero new information, so per the lane's own precedent (Capterra
2026-08-15: "No ops doc touched — avoiding a duplicate conflicting PR on the
shared file"; Microlaunch 2026-08-17: same pattern) this run lands only its
unique verification receipt.

## Changes made

- `.lane/reports/lane1-uneed-listing-20260817.md`: this per-branch report
