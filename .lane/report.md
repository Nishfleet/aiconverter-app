...
### What this lane shipped (PR #44 → superseded by this PR, branch `lane1/indexnow-gitleaks-unblock`)
...
- `.gitleaks.toml` — allowlists the by-design-public IndexNow key so the
  Secret Scan gate (Gitleaks `generic-api-key` false positive on
  `ops/bing-indexation.md:87`) no longer blocks the deliverable.
